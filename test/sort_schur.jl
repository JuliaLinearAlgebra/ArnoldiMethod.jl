...
using LinearAlgebra
using Base: OneTo
using ArnoldiMethod: Rotation2, Rotation3, NotWanted, get_rotation
...
@inline is_start_of_11_block(R, i) = i == size(R, 1) || @inbounds(iszero(R[i+1,i]))
@inline is_end_of_11_block(R, i) = i == 1 || @inbounds(iszero(R[i,i-1]))

function example(n = 10)
    # Generate a random orthonormal matrix
    QQ = qr(rand(n, n)).Q * Matrix(I, n, n)

    # Generate a quasi upper triangular matrix
    RR = triu(rand(n, n))
    RR[4,3] = rand()
    RR[3,4] = -rand()
    RR[7,8] = rand()
    ...
    # Find the corresponding A that induces the Schur decomp AQ=QR
    A = QQ * RR * QQ'

    Q, R = copy(QQ), copy(RR)

    rotate_left!(R, 3, 7, Q)
    rotate_right!(R, 1, 5, Q)
